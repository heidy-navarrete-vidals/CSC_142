...
int main() {
    // Objects w/parameters
    Book bookObj1("013478796X", "Tony Gaddis", 
                "Starting Out With Java: From Control Structures through Data Structures",
                4, "PE", 118.30);

    Book bookObj2("0321409493", "John Lewis", 
                "Java Software Solutions: Foundations of Program Design", 
                5, "AW", 94.05);
    
    Book bookObj3("0134743350", "Harvey Deitel", "Java How to Program, Early Objects",
                12, "PE", 134.84);
    ...
    printf("%-13s %-16s %-43.40s %2d %4s $%8.2f\n",
        bookObj1.getISBN().c_str(),
        bookObj1.getAuthor().c_str(),
        bookObj1.getTitle().c_str(),
        bookObj1.getEdition(),
        bookObj1.getPubCode().c_str(),
        bookObj1.getPrice());
    
    printf("%-13s %-16s %-43.40s %2d %4s $%8.2f\n",
        bookObj2.getISBN().c_str(),
        bookObj2.getAuthor().c_str(),
        bookObj2.getTitle().c_str(),
        bookObj2.getEdition(),
        bookObj2.getPubCode().c_str(),
        bookObj2.getPrice());

    printf("%-13s %-16s %-43.40s %2d %4s $%8.2f\n",
        bookObj3.getISBN().c_str(),
        bookObj3.getAuthor().c_str(),
        bookObj3.getTitle().c_str(),
        bookObj3.getEdition(),
        bookObj3.getPubCode().c_str(),
        bookObj3.getPrice());
    ...
    // Changing Price Function
    bookObj1.changePrice(145.68);
    bookObj2.changePrice(117.79);
    bookObj3.changePrice(59.99);
